...
def fast_prune(edge_index, dists, num_nodes, hard_cutoff, poly_exp, sig_exp, weight_mean, weight_std):
    ...
    return keep, cutoffs.unsqueeze(1)
...
def prepare_graph(
    ...
    cutoffs = torch.tensor(r_max, device=lengths.device)

    edge_index = data['edge_index']
    if isinstance(edge_index, np.ndarray):
        edge_index = torch.from_numpy(data['edge_index']).to(device=lengths.device)

    if prune:
        keep, cutoffs = fast_prune(edge_index, lengths, num_nodes, r_max, poly_exp, sig_exp, weight_mean, weight_std)
