...
class PolynomialCutoff(torch.nn.Module):
    ...
    def forward(self, x: torch.Tensor, cutoffs: torch.Tensor) -> torch.Tensor:
        return self.calculate_envelope(x, cutoffs, self.p.to(torch.int))

    @staticmethod
    def calculate_envelope(
        x: torch.Tensor, cutoffs: torch.Tensor, p: torch.Tensor
    ) -> torch.Tensor:
        r_over_r_max = x / cutoffs
        envelope = (
            1.0
