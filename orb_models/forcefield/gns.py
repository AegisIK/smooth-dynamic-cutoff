...
from orb_models.forcefield.nn_util import build_mlp, get_cutoff, mlp_and_layer_norm, get_dyn_envelope
...
from orb_models.forcefield.atomic_system import fast_prune
...
class MoleculeGNS(nn.Module):
    ...
    def forward(self, batch: base.AtomGraphs) -> Dict[str, torch.Tensor]:
        ...
        if self.dynamic_cutoff:
            cutoffs, keep = fast_prune(batch.senders, batch.receivers, batch.n_node.sum(), batch.edge_features["vectors"].norm(dim=-1), 6.0, 50, 10, 40, 4)
            batch.edge_features["vectors"] = batch.edge_features["vectors"][keep]
            batch.edge_features["unit_shifts"] = batch.edge_features["unit_shifts"][keep]

            senders = batch.senders[keep]
            receivers = batch.receivers[keep]
        else:
        ...
        if self.conditioner is not None:
            # No conditioners, we don't worry about anything but energy/forces
        ...
        cutoff = get_dyn_envelope(batch.edge_features["vectors"].norm(dim=-1), cutoffs) if self.dynamic_cutoff else get_cutoff(batch.edge_features["vectors"].norm(dim=-1))
    ...
    def featurize_edges(self, batch: base.AtomGraphs, cutoffs = None) -> torch.Tensor:
        ...
        if self.outer_product_with_cutoff:
            cutoff = get_cutoff(lengths) if cutoffs is None else get_dyn_envelope(lengths, cutoffs)
            # (nedges, x, y)
