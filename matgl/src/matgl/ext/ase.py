...
class PESCalculator(Calculator):
    ...
    implemented_properties = ["energy", "free_energy", "forces", "hessian", "magmoms"]  # noqa:RUF012
    ...
    def calculate(  # type:ignore[override]
        ...
        graph_times.append(perf_counter() - start)
        # type: ignore
        ...
        # print("returning pes forward")
        # return calc_result

        multiplier = 0.043641 if self.md22 else 1

        self.results.update(
            energy=calc_result[0].detach().cpu().numpy().item() * multiplier,
            free_energy=calc_result[0].detach().cpu().numpy().item() * multiplier,
            forces=calc_result[1].detach().cpu().numpy() * multiplier,
        )
    ...
    def relax(
        ...
        **kwargs,
        ...
        with contextlib.redirect_stdout(stream):
            ...
            self.calculator.implemented_properties = ["energy", "forces"]
...
class TrajectoryObserver(collections.abc.Sequence):
    ...
    def __call__(self) -> None:
        ...
        self.stresses.append(np.zeros((3, 3)))
...
class MolecularDynamics:
    ...
    def __init__(
        ...
        self.atoms.set_calculator(
            PESCalculator(potential=potential, state_attr=state_attr, stress_unit="eV/A3", stress_weight=stress_weight, md22=md22)
        )
